...
# DISTANCES = [50., 10., 4.21, 2.59, 2.0, 1.74, 1.36, 1.07, 0.88, 0.7, 0.59, 0.5]
DISTANCES = [50., 6.0, 3.0, 2.0, 1.5, 1.2, 1., 0.85, 0.75, 0.66, 0.6, 0.55, 0.5]
# diopters with 0.2 spacing
# DISTANCES = [50., 5., 2.5, 1.67, 1.25, 1., 0.83, 0.71, 0.63,  0.56, 0.5]
NUM_DISCRETE_DEPTHS = 2

# Set a fixed log dir outside of repository.
LOGDIR = '/graphics/scratch/engelhar/train_logs/model_slm45k_2prop_unet4_oryx_inf_rot_ddepth_alpha_ssim'
...
EXPORT_PATH = '/graphics/scratch/engelhar/test_data/unet4_slm45k_2prop_oryx_inf_rot_ddept_halpha_ssim'
...
def generate_poly_coordinates(imshape, no_of_masks=1):
    vertices_list=[]
    for index in range(no_of_masks):
        vertex=[]
...
def add_alpha_matte(image, no_of_masks=1):
    imshape = image.shape
    mask = np.zeros((imshape[0], imshape[1], 1), dtype=np.uint8)
    vertices_list= generate_poly_coordinates(imshape, no_of_masks)
    for vertices in vertices_list:
        cv2.fillPoly(mask, vertices, 1)
    image_with_alpha = np.concatenate((image, mask), 2)
...
class Model(ModelDesc):
    ...
    def _get_inputs(self):
        return [InputDesc(tf.float32, (None, INSHAPE, INSHAPE, CHANNELS+(NUM_DISCRETE_DEPTHS-1)), 'incoming'),
                InputDesc(tf.float32, (None, INSHAPE, INSHAPE, CHANNELS), 'expected'),
                InputDesc(tf.float32, (None, NUM_DISCRETE_DEPTHS), 'distance')]
    ...
    def _encoder(self, incoming, depth):
        ...
        with tf.name_scope('optics_sim'):
            ...
            # A depth value is generated for each NUM_DISCRETE_DEPTHS.
            if get_current_tower_context().is_training:
                d_idxs = tf.random_uniform([NUM_DISCRETE_DEPTHS], 0.0, 1.0, dtype=tf.float32)
                d_idxs = tf.cast(d_idxs * (len(DISTANCES) - 1), tf.int32, name='cast_distanceIdx')
                distance_tensor = tf.convert_to_tensor(DISTANCES)
                distance = tf.map_fn(lambda d: tf.gather(distance_tensor, d, name='gather_d'), d_idxs, dtype=tf.float32)
                ...
                d_idxs = tf.cast(d_idxs * (len(DISTANCES) - 1), tf.int32, name='cast_distanceIdx')
                distance_tensor = tf.convert_to_tensor(DISTANCES)
                distances = tf.map_fn(lambda d: tf.gather(distance_tensor, d, name='gather_d'), d_idxs, dtype=tf.float32)
                distance = tf.cond(tf.equal(depth[0, 0], 0.0), lambda: distances,
                                   lambda: depth, name='cond_distance')


            # Tell tf the shape of the distance in case it got lost during the conditional.
            distance.set_shape(NUM_DISCRETE_DEPTHS)
            # Tensorboard output.
            add_moving_summary(distance[0])
            add_moving_summary(distance[1])
            ...
            for c in range(0, CHANNELS):
                image_layer = tf.zeros([BATCH, SHAPE, SHAPE, 1], name='image_layer_c%s' % c)
                psf_layer = None
                for d in range(0, NUM_DISCRETE_DEPTHS):
                    ...
                    if RND_WVL:
                        ...
                        wvl = tf.convert_to_tensor(WAVELENGTHS[c], dtype=tf.float32, name='wavelength_c%s_d%s' % (c, d))
                    ...
                    if CURVATURE_APPROX:
                        phi_d = np.pi / (wvl * distance[d]) * squared_sum
                        field_o = tf_exp_complex(phi_d, 'exp_phase_o_c%s_d%s' % (c, d))
                    else:
                        ...
                        single_distance = tf.cast(distance[d], dtype=tf.float64)
                        r = tf.sqrt(squared_sum2 + single_distance * single_distance, name='sqrt_r_c%s_d%s' % (c, d))
                    ...
                    if SLM_CROP:
                        ...
                        field_o = tf.multiply(field_o, crop, name='mult_field_0_crop_c%s_d%s' % (c, d))

                    # If zeroth order diffraction peak is to be simulated, create an unmodulated field as well.
                    if ZEROTH_ORDER > 0:
                        field_unmodulated = field_o
                        field_o = tf_exp_complex(phi_slm, 'exp_phase_o_c%s_d%s' % (c, d)) * field_o
                    else:
                        field_o = tf_exp_complex(phi_slm, 'exp_phase_o_c%s_d%s' % (c, d)) * field_o
                    ...
                    with tf.name_scope('OpticalElement_Lens'):
                        ...
                        field_o = tf_exp_complex(phi_lens, 'exp_phase_lens_c%s_d%s' % (c, d)) * field_o

                        # Effect of aperture.
                        pupil_function = tf.multiply(tf.complex(aperture, 0.0, name='complex_aperture_c%s_d%s' % (c, d)),
                                                     field_o,
                                                     name='mult_pupilFunction_c%s_d%s' % (c, d))
                    ...
                    uout = tf.abs(uout, name='abs_hMagnitude_c%s_d%s' % (c, d))
                    psf = tf.square(uout, name='square_hSquared_c%s_d%s' % (c, d))
                    ...
                    if ZEROTH_ORDER > 0:
                        field_unmodulated = ang_fresnel_prop_variable_padded(field_unmodulated, Z1_DISTANCE, delta,
                                                                             ...
                                                                             SHAPE, padding)
                        field_unmodulated = tf_exp_complex(phi_lens, 'exp_phase_lens_c%s_d%s' % (c, d)) * field_unmodulated
                        field_unmodulated = tf.multiply(tf.complex(aperture, 0.0, name='complex_aperture_c%s_d%s' % (c, d)),
                                                        field_unmodulated, name='mult_pupilFunction_c%s_d%s' % (c, d))
                        ...
                        uout_unmodulated = tf.abs(uout_unmodulated, name='magnitude_h_c%s_d%s' % (c, d))
                        psf_unmodulated = tf.square(uout_unmodulated, name='hSquared_c%s_d%s' % (c, d))
                    ...
                    psf = tf.divide(psf, tf.reduce_sum(psf), name='norm_psf_c%s_d%s' % (c, d))
                    psf_layer = psf
                    ...
                    if RND_TRANSFORM > 0:
                        psf = random_perturbation(psf, RND_TRANSFORM, name='random_transform_c%s_d%s' % (c, d))

                    # Convolve using custom fft implementation
                    cout = tf_conv2d(incoming[:, :, :, c], psf, SHAPE, SHAPE - 2 * crop)
                    cout = tf.expand_dims(cout, -1, name='expand_fftOut_c%s_d%s' % (c, d))
                    if d == 0:
                        image_layer = cout
                    else:
                        alpha = incoming[:, :, :, CHANNELS:CHANNELS+d]

                        image_layer = image_layer * (1.0 - alpha) + cout * alpha
                psfs.append(psf_layer)
                coded_image.append(image_layer)
        ...
        with tf.name_scope('Sensor'):
            ...
            sigma = tf.random_uniform([1], 0.005, 0.018, dtype=tf.float32)
    ...
    def _decoder(self, enc_activations):
        ...
        with tf.name_scope('decoder'):
            # Only pass along the image information - blind deconvolution.
    ...
    def _build_graph(self, inputs):
        ...
        #                    name='reg_mse')
        self.cost = tf.add(-1. * tf.reduce_mean(tf.image.ssim_multiscale(prediction, expected, 1.0), name="msssim"),
                           1e-8 * variation, name='reg_loss')
...
def get_data(subset='train'):
    ...
    if subset == 'export':
        ...
        ds = MapData(ds, lambda dp: [add_alpha_matte(dp[0]), dp[0], np.zeros([NUM_DISCRETE_DEPTHS])])
    # Create batches.
    ds = PrintData(ds)
...
class OutputSLM(Callback):
    ...
    def _after_train(self):
        for var in self.varlist:
            ...
            output = "./image_SLM45k_2prop_zo_unet4_oryx_d2_rot_ddepth_alpha_ssim.png"
    ...
    def _trigger(self):
        ...
        for input, expected in self.test_ds.get_data():
            ...
            for d in self.distances:
                outputs = self.pred(input, expected, np.ones([1, NUM_DISCRETE_DEPTHS], dtype=np.float32)*float(d))
                im = outputs[0]
                # Write image to disk.
...
def get_config():
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            InferenceRunner(dataset_val, [ScalarStats(['msssim:0', 'psnr/prediction:0', 'psnr/ssim:0'])]),
            ...
            # Switched off because of different distances format
            PeriodicTrigger(TestDataExport(), every_k_epochs=50)
        ],
        extra_callbacks=[
            MovingAverageSummary(),
            ProgressBar(['msssim:0', 'psnr/prediction:0']),
            MergeAllSummaries(),
            OutputSLM()
        ],
        model=Model(),
