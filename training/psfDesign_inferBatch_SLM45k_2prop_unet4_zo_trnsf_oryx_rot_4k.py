...
def _encoder(incoming, depth):
    ...
    with tf.name_scope('optics_sim'):
        ...
        quadrant1 = tf.get_variable('a_slm', [SLM_RES[1] // 2, SLM_RES[0] // 2],
    ...
    return incoming, activations[:, :, :, 1:], slm_viz
...
def inference(input, model=LOGDIR, d=100.0):
    ...
    with tf.Graph().as_default():
        with TowerContext('tower0', is_training=False):
            ...
            x = tf.placeholder('float32', [None, 4096, 4096, CHANNELS])
            # x = tf.tile(x, [4, 1, 1, 1], name='tile_input')

            # build graph
            activations, psfop, slm = _encoder(x, d)
            activations = tf.concat([activations, activations], 3)
            ...
            with tf.Session() as sess:
                ...
                # run session
                output, enc_out, psfimg, psnrm, ssim = sess.run([prediction, activations, psfop, psnr_pred, ssim_pred],
                                                                   feed_dict={x: input})
                # lucy richardson
                # output = lucyrichardson_deconvolution.lr_deconv(enc_out[0, :, :, 0], enc_out[0, :, :, 1])

                return output[0], psfimg, slm_p, psnrm, ssim
