...
L_SENSOR = 3.45e-6 * 2048  # set to the sensor size of a FLIR Oryx 4K camera.
...
LOGDIR = '/graphics/scratch/engelhar/train_logs/model_slm45k_2prop_unet4_oryx-half_inf_rot_ddepth'
...
EXPORT_PATH = '/graphics/scratch/engelhar/test_data/unet4_slm45k_2prop_oryx-half_inf_rot_ddepth'
        ...
        with tf.name_scope('optics_sim'):
            ...
            for c in range(0, CHANNELS):
                ...
                for d in range(0, NUM_DISCRETE_DEPTHS):
                    ...
                    depth_layer_height = cout.get_shape().as_list()[1] // NUM_DISCRETE_DEPTHS
                    image_layer = tf.concat([image_layer[:, 0:d*depth_layer_height, :, :],
                                             cout[:, d*depth_layer_height:(d+1)*depth_layer_height, :, :]], 1,
                                            name='concat_layer_c%s_d_%s' % (c, d))
...
def get_data(subset='train'):
    ...
    if subset == 'export':
        ...
        ds = MapData(ds, lambda dp: [dp[0], dp[0], np.zeros([NUM_DISCRETE_DEPTHS])])
    # Create batches.
    ds = PrintData(ds)
...
class OutputSLM(Callback):
    ...
    def _after_train(self):
        for var in self.varlist:
            ...
            output = "./image_SLM45k_2prop_zo_unet4_oryx_d2_rot_ddepth.png"
