...
# Framework for Simulation of SLM driven computational camera applications and reconstruction v0.3
...
FOCUS_DISTANCE = 2.0  # distance of focus plane from pupil of optical element in m, -1 for infinity
...
Z2_DISTANCE = 0.1049  # distance between SLM and lens in m - this is an approximated, measured value
...
LOGDIR = '/graphics/scratch/engelhar/train_logs/model_slm45k_2prop_no-psf_d2_unet4_oryx'
# LOGDIR = '../models/model_slm2k_2proptest'
...
EXPORT_PATH = '/graphics/scratch/engelhar/test_data/unet4_slm45k_2prop_no-psf_d2_oryx'
# EXPORT_PATH = '../test_data/plainCNN_slm2k_2proptest'
...
class Model(ModelDesc):
    ...
    def _encoder(self, incoming, depth):
        ...
        with tf.name_scope('optics_sim'):
            ...
            # sample rate
            # SLM first, adding padding around the real size of SLM
            padding = PAD
        ...
        with tf.name_scope('Sensor'):
            ...
            sigma = tf.random_uniform([1], 0.0, 0.02, dtype=tf.float32)
    ...
    def _decoder(self, enc_activations):
        ...
        with tf.name_scope('decoder'):
            dec_out = reconstruction.unet4(enc_activations[:, :, :, 0:1], CHANNELS, FEATURE_FACTOR)
            return dec_out
    ...
    def _build_graph(self, inputs):
        ...
        # self.cost = tf.reduce_mean(tf.squared_difference(prediction, expected), name="mse")
...
class OutputSLM(Callback):
    ...
    def _after_train(self):
        for var in self.varlist:
            ...
            output = "./image_SLM45k_2prop_no-psf_d2_unet4_oryx.png"
...
def get_config():
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            ScheduledHyperParamSetter('learning_rate', [(8, 2e-4), (50, 1e-4), (95, 1e-4)]),
