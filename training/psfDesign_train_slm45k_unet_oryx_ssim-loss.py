...
DISTANCES = [50., 6.0, 3.0, 2.0, 1.5, 1.2, 1., 0.85, 0.75, 0.67, 0.6, 0.55, 0.5]
# DISTANCES = [50, 5, 2, 1, 0.67, 0.5]

# Set a fixed log dir outside of repository.
LOGDIR = '/graphics/scratch/engelhar/train_logs/model_slm45k_2prop_unet4_oryx_d2_ssim-loss'
...
EXPORT_PATH = '/graphics/scratch/engelhar/test_data/unet4_slm45k_2prop_oryx_d2_ssim-loss'
...
def laplacian_filter_tf(img_batch):
    ...
    img_batch = tf.expand_dims(img_batch, -1)
    ...
    return filtered_batch[0, :, :, 0]


# using a scale of 1000 is reasonabe for free form optical element
...
class Model(ModelDesc):
    ...
    def _encoder(self, incoming, depth):
        ...
        with tf.name_scope('optics_sim'):
            ...
            with tf.name_scope('OpticalElement_SLM'):
                ...
                phi_slm = tf.get_variable('a_slm', [SLM_SHAPE[1], SLM_SHAPE[0]], initializer=tf.zeros_initializer)
                # Register in collection to apply clipping after each iteration.
    ...
    def _build_graph(self, inputs):
        ...
        self.cost = tf.add(-1. * tf.reduce_mean(tf.image.ssim_multiscale(prediction, expected, 1.0), name="msssim"),
                           tf.multiply(regularize_cost("a_slm", regularizer), reg), name='reg_loss')
...
class OutputSLM(Callback):
    ...
    def _after_train(self):
        for var in self.varlist:
            ...
            output = "./image_SLM45k_2prop_zo_unet4_oryx_d2_ssim-loss.png"
...
def get_config():
    ...
    return TrainConfig(
        ...
        callbacks=[
            ...
            ScheduledHyperParamSetter('learning_rate', [(8, 2e-4), (40, 5e-4), (95, 1e-4)]),
            # ScheduledHyperParamSetter('regularization_factor', [(12, 0.001), (40, 0.01), (95, 0.03)]),
