...
from psfDesign_train_multifocal_fixed_blendmap_simple_separation import Model
...
if ___name__ == "__main__":
    """
    inference with tensorpack method"""
    pred = OfflinePredictor(PredictConfig(
        session_init=get_model_loader(),
        model=Model(),
        input_names=['wide_angle_view'],
        output_names=['separated_two_views']))

    w_img = imread("")
    output = pred(w_img)
    score = psnr(output[:,:,0], w_img)
